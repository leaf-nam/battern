...
- **폐곡선 저장 제한**: 모든 선/곡선의 끝점이 다른 하나의 끝점과 정확히 맞닿아야만(연결 차수 = 2) "SVG 저장"과 "보관함에 저장"이 가능합니다. 끝점을 다른 끝점 근처(0.6cm 이내)로 그리거나 드래그하면 자동으로 스냅되어 붙고, 아직 연결되지 않은 점은 캔버스에 빨간 링으로 표시됩니다. PNG 내보내기·인쇄는 작업 중인(열린) 패턴도 확인용으로 계속 사용할 수 있습니다.
